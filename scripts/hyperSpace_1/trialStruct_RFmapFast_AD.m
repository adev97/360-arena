...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [trials] = trialStruct_RFmapFast_AD( stimType, table)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=1:(constants.Repeats+1)
        ...
        if constants.Randomize == 1 && constants.Interleave ~=1
            ...
            if strcmp(stimType,'Receptive Field Mapping')
                trial_num = size(trial_arrays,1);
                randIndex = zeros(1,trial_num);
                IndexPool = 1:trial_num;
                previous_positionIdx = 0;
                for k=1:trial_num
                    count_n = 0;
                    while 1
                        curtempIdx = ceil(rand*length(IndexPool));
                        if curtempIdx ==0
                            curtempIdx=1;
                        end
                        curIdx = IndexPool(curtempIdx);
                        current_positionIdx = round(curIdx/2);
                        if current_positionIdx ~= previous_positionIdx
                            break;
                        end
                        count_n = count_n + 1;
                        if count_n > 10
                    ...
                    randIndex(k) = curIdx;
                    IndexPool(curtempIdx)=[];
                    previous_positionIdx = current_positionIdx;
                end
        ...
        trial_arrays_cat = vertcat(trial_arrays_cat, trial_arrays);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % No parameters to vary -- e.g. every table row is a constant, as in
    % a single-condition optic-flow run. Without this branch, trials is
    % never created, and the constants-assignment loop below (which uses
    % comma-list assignment trials(:).field = deal(...)) fails with
    % "Comma-separated list assignment to a nonexistent variable is not
    % supported when any index is a colon." Initializing trials as a
    % scalar struct here gives that loop something to attach fields to.
    trials = struct();
end
