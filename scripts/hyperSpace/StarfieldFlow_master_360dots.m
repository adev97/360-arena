...
p.Travel_Speed = 90;             % cm/s; 180 doubles forward speed
...
stimulusTable = { ...
    ...
    'Timing (delay,duration,wait)', 0, 30, 3; ...
    'Blank',                       0, [], []; ...
    'Randomize',                   0, [], []; ...
    'Interleave',                  0, [], []; ...
...
meta.photodiodeMeaning = ['LeftBoxStim_small box toggles white/black on every logged ', ...
    'flip (delay, stimulus, offset, ITI), one toggle across the whole run; compare ', ...
    'against a physically recorded diode trace to catch dropped frames.'];
...
for k = 1:numel(sourceNames)
    resolvedPath = which(sourceNames{k});
    assert(~isempty(resolvedPath), ...
        'Could not locate %s on the MATLAB path.', sourceNames{k});
    meta.sourceText{k} = fileread(resolvedPath);
    meta.sourcePaths{k} = resolvedPath;
end
