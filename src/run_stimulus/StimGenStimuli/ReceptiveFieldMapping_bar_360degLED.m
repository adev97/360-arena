...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function ReceptiveFieldMapping_bar_360degLED(trials)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %This function generates black or white full-height bar sequences with parameters
    ...
    try
        ...
        for trial = 1:numel(trials)
            ...
            elseif flagLuminance == 0.5
                SquareLum = grayPix;
            end

            % Convert bar width and center position to pixel units. The
            % half-pixel screen center keeps the requested even pixel widths
            % symmetric at all 30 centers.
            pxSize = ceil(degSize / degPerPix);
            pxPositionX = degPositionX / degPerPix;
            pixCenterX = (monitorInfo.screenSizePixX + 1) / 2 + pxPositionX;

            % Square_Size is the bar width. The bar fills the complete Y axis.
            pixXlim(1) = round(pixCenterX - (pxSize - 1) / 2);
            pixXlim(2) = pixXlim(1) + pxSize - 1;
            pixYlim(1) = 1;
            pixYlim(2) = monitorInfo.screenSizePixY;
            ...
            if ~strcmp(trials(trial).Stimulus_Type, 'Blank')
                ...
                while (vbl < runtime)
                    ...
                    % rectangle
                    Screen('DrawTextures', w, squaretex{trial}, screenRect, ...
                        screenRect);
