...
% Reason for comment-out: keep the output signature used by
% visualStimuli_rotation_v2 while saving RF-map trials plus bg pixel offset.
% function ReceptiveFieldMapping_Fast_360degLED_Rotate_v2(trials)
function trials = ReceptiveFieldMapping_Fast_360degLED_Rotate_v2(trials, staticBG, bgTurn)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    try
        ...
        % in luminance
        if trials(1).Initialization_Screen > 0
        ...
        for barIdx = 1:numel(barCenters)
            x1 = round(barCenters(barIdx) - barW / 2) + 1;
            x2 = x1 + barW - 1;
            baseMtrx(:, x1:x2) = grayPix;
        end

        wrappedBaseMtrx = [baseMtrx baseMtrx];
        movingTex = Screen('MakeTexture', w, wrappedBaseMtrx);
        preVsStaticBackgroundSec = staticBG;
        preVsBackgroundOnlyTurns = bgTurn;
        % Use the legacy average speed: one background pixel per two RF
        % trial durations. One turn is one full texture wrap across W pixels.
        preVsTurnDurationSec = W * 2 * trials(1).Timing(2);

        [trials(:).BackgroundRotation_XOffset_Pix] = deal(NaN);

        vbl = Screen('Flip', w);
        staticEndTime = vbl + preVsStaticBackgroundSec;

        while (vbl < staticEndTime)
            Screen('DrawTextures', w, movingTex, [0 0 W H], screenRect);
        ...
        motionStart = vbl;
        rotationOnlyEndTime = motionStart + preVsBackgroundOnlyTurns * preVsTurnDurationSec;

        while (exitLoop ~= 1 && vbl < rotationOnlyEndTime)
            [~, ~, srcRect] = timedRotationGeometry(vbl, motionStart, ...
                preVsTurnDurationSec, W, H);
        ...
        for trial = 1:numel(trials)
            ...
            % Old gray-background square texture path kept for reference.
            % The active path builds one full frame from the rotated background
            % plus this static RF square, then draws that frame to screenRect.
            ...
            if ~strcmp(trials(trial).Stimulus_Type, 'Blank')
                ...
                trials(trial).BackgroundRotation_XOffset_Pix = xoffset;
                vsIndicatorCount = vsIndicatorCount + 1;
                imMtrx = wrappedBaseMtrx(:, xoffset + (1:W));
                drawX = max(1, pixXlim(1)):min(W, pixXlim(2));
                drawY = max(1, pixYlim(1)):min(H, pixYlim(2));
                if ~isempty(drawX) && ~isempty(drawY)
                    imMtrx(drawY, drawX) = SquareLum;
                end
                stimTex = Screen('MakeTexture', w, imMtrx);

                % Set the runtime of each trial by adding duration to vbl time
                runtime = vbl + duration;

                while (vbl < runtime)
                    % Draw one full logical frame to the full PTB window.
                    Screen('DrawTextures', w, stimTex, [0 0 W H], screenRect);
...
function [motionStep, xoffset, srcRect] = timedRotationGeometry(vbl, motionStart, ...
        turnDurationSec, W, H)
    elapsed = max(0, vbl - motionStart);
    motionStep = mod(elapsed / turnDurationSec * W, W);
    xoffset = mod(round(W - motionStep), W);
